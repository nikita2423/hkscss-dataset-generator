  ...
  const handleFileProcessing = useCallback(
    async (files, pdfStrategy, selectedViosnModel, domainTreeAction) => {
      try {
        ...
        //   localStorage.getItem("modelConfigList")
        // );
        // const vsionModel = availableModels.find(
        //   (m) => m.id === selectedViosnModel
        // );
        ...
        if (response.data?.code !== 0) {
          throw new Error(
            "Pdf Processing failed" + (response.data?.error || "")
          );
        }

        //提示后台任务进行中
        toast.success("parsing is in progress in background");
      } catch (error) {
        toast.error("PDF processing failed" + error.message || "");
      }
    },
    [projectId, resetProgress]
  );
