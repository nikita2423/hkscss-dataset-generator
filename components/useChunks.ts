  ...
  const fetchChunks = useCallback(
    async (filter = "all") => {
      try {
        ...
        // 如果有文件结果，处理详细信息
        if (data.toc) {
          console.log("File result received", data.fileResult);
          // 如果有目录结构，设置目录数据
          setTocData(data.toc);
        }
      } catch (error) {
    ...
    [projectId, setLoading, setChunks, setTocData]
  );
  ...
  const handleEditChunk = useCallback(
    async (chunkId, newContent) => {
      try {
        ...
        const response = await fetch(
          `/api/projects/${projectId}/chunks/${encodeURIComponent(chunkId)}`,
          {
            method: "PATCH",
            headers: {
        ...
        toast.success("Text chunk edited successfully");
      } catch (error) {
        toast.error(error.message);
      } finally {
        setLoading(false);
